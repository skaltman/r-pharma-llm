...
::: notes
Let's revisit how LLMs work. This is important context for understanding why we need tools.
:::
...
::: notes
Let's try asking about things the model doesn't know. This one fails - the model's training data is old and it doesn't have real-time event information.
:::
...
::: notes
But this one... the model actually gives an answer! It's wrong (the date is from its training cutoff), but it shows the model will try to answer with what it knows.
:::
...
The model doesn't have access to real-time weather data, just like we saw earlier with Minneapolis.
:::
...
::: notes
The LLM also chooses the arguments for the tool. Here it picks the location based on the conversation.
:::
...
::: notes
It uses its knowledge to fill in the right values - using information from its training data.
:::
...
::: notes
The LLM can also call multiple tools in a single response if it needs to gather different information.
:::
...
::: notes
Let me show you a Shiny app where you manually approve each tool call. This shows the pattern clearly - you're always in control.
:::
...
::: notes
Here's the pattern for defining tools in ellmer. You provide the function, a description, and argument types.
:::
...
# Your Turn `10_quiz-game-2` {.slide-your-turn}

::: notes
Now practice - add a tool to the quiz game that plays sounds. This reinforces the pattern we just learned.
:::
...
::: notes
When using tools in Shiny apps, remember these key points - tools can interact with your app's reactive state.
:::
...
::: notes
useful if you want to update a table based on chat input
:::
